...
//            String newFileName = this.getName(fileName);
            String newFileName = request.getParameter("desc");
            // 文件扩展名
            ...
            //daidaidaidai修改
            String pictureName = "/media/wxc/B66C7B026C7ABCA5/AppInUbuntu/IDEA/spoc-manager-web/src/main/webapp/picture/" + name+".jpg";
            System.out.println("保存的绝对路径" + fileNamedirs);
            System.out.println("图片的保存路径"+pictureName);
            File filedirs = new File(fileNamedirs);

            boolean transfer =FileUploadTool.transfer(fileNamedirs,pictureName,"0");
            // 转入文件
            try {
            ...
            if (aviPath != null) {
                ...
                if (this.checkMediaType(fileEnd)) {
                    try {
                        ...
                        String ffmpegPath = request.getSession().getServletContext().getRealPath("/tools/ffmpeg.bat");
